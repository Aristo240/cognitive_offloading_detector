...
ax = axes[0]
bars = ax.bar(x, rs, color=["#4C72B0", "#55A868", "#C44E52"], edgecolor="black")
ax.set_xticks(x)
ax.set_xticklabels(labels, fontsize=11, rotation=20, ha="right")
ax.set_ylabel("Aggregate Pearson r", fontsize=12)
ax.set_title("3-judge cross-family agreement\n(UltraChat-200k, n=92-100)", fontsize=13)
ax.set_ylim(0, 0.55)
for i, v in enumerate(rs):
    ax.text(i, v + 0.015, f"{v:.2f}", ha="center", fontsize=13, fontweight="bold")

ax = axes[1]
bars = ax.bar(x, ks_ac, color=["#4C72B0", "#55A868", "#C44E52"], edgecolor="black")
ax.set_xticks(x)
ax.set_xticklabels(labels, fontsize=11, rotation=20, ha="right")
ax.set_ylabel("Cohen's κ (quadratic)", fontsize=12)
ax.set_title("answer_copying agreement (κ)", fontsize=13)
ax.set_ylim(-0.05, 0.55)
ax.axhline(0.0, color="black", linewidth=0.5)
for i, v in enumerate(ks_ac):
    ax.text(i, v + 0.015 if v >= 0 else v - 0.04, f"{v:.2f}",
...
files = [
    ...
    ("Llama-3.3-70B-FP8\n(open weights, vLLM)", "vllm__Llama-3.3-70B-Instruct-FP8.jsonl", "#C44E52"),
]
for ax, (name, fname, color) in zip(axes, files):
    ...
    for line in path.read_text().splitlines():
        ...
        try:
            ...
            continue
    aggs = np.array(aggs)
    ax.hist(aggs, bins=20, range=(0, 2), color=color, edgecolor="black", alpha=0.85)
    ax.set_xlim(0, 2)
    ax.set_xlabel("aggregate offloading score (0=think-with, 2=offload)", fontsize=11)
    mean = aggs.mean() if len(aggs) else 0
    ax.set_title(f"{name}\nn={len(aggs)},  mean={mean:.2f}", fontsize=13)
    ax.axvline(mean, linestyle="--", color="black", alpha=0.7, linewidth=1.5)
    ax.tick_params(axis="both", labelsize=11)
axes[0].set_ylabel("conversations", fontsize=12)
fig.suptitle("Per-judge distributions on the SAME 100 UltraChat conversations  —  Gemini's all-zero pattern is the disagreement source",
             fontsize=13)
fig.tight_layout()
